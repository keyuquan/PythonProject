...
都具有 w‘x+b,其中w和b是待求参数,其区别在于他们的因变量不同,多重线性回归直接将w‘x+e作为因变量,即 y =w‘x+e,
而logistic回归则通过函数L将w‘x+b对应一个隐状态p,p =L(w‘x+e),然后根据p 与1-p的大小决定因变量的值。如果L是logistic函数,
就是logistic回归,如果L是多项式函数就是多项式回归,机器学习的 l 一般为  sigmod函数 L=1/(1+e^(-y)),p=w‘x+b；y=1/(1+e^(-(w‘x+e；)))
...
即是 确定 关系式子 y = w'x+e 和 p=1/(1+e^(-(w‘x+e；))) 的参数值
